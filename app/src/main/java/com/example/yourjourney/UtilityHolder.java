...
public class UtilityHolder extends RecyclerView.Adapter<UtilityHolder.MyViewHolder>  {

    Context context;
    ArrayList<Utility> utilities;


    public UtilityHolder(Context context, ArrayList<Utility> utilities) {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        Utility utilit = utilities.get(position);
        holder.rTitle.setText(utilit.getTitle());
        Picasso.get().load(utilit.getImage()).into(holder.rImage);
        holder.rButton.setOnClickListener(new View.OnClickListener() {

            @Override
            public void onClick(View view) {
                String googleMapsLink = utilit.getMaps();
                Intent viewIntent =
